...
// Device viewport sizes for a manual Small/Medium preview (see
...
const QSize kSmallViewportSize(360, 700);
// A real Android tablet's portrait size (800x1280 dp). Must stay wider than
// kSmallBreakpointMaxViewportWidth + kBreakpointHysteresisPx
// (mainwindow.cpp) so the width previewed as Medium is also the width a
// real device auto-detects as Medium -- the earlier 600x1000 fell inside
// Small's range, so a portrait tablet got the Small layout instead.
const QSize kMediumViewportSize(800, 1280);
...
// resize a manual Small/Medium preview used to do (see MainWindow::
...
public:
    ...
    // fills the frame's whole rect(), pixel for pixel -- Large. A valid
    // size requests that device's own viewport (Small/Medium); BOTH its
...
protected:
    ...
    // device with a 1px bezel -- a no-op when there's no frame (Large),
...
private:
    ...
    QSize m_deviceSize;  // invalid QSize() = no frame (Large)
