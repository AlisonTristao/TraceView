...
RobotLogConfigEditor::RobotLogConfigEditor(QWidget* parent) : WidgetConfigEditor(parent) {
    ...
    auto* heading = new QLabel(tr("Tabs — one log per device"), this);
    ...
    addButton->setToolTip(tr("Add another log tab for a device."));
...
void RobotLogConfigEditor::setConfig(const QJsonObject& config) {
    ...
    // A no-op guard, same as RobotLogWidget::setConfig()'s own
    // `if (deviceIds == m_deviceIds) return;` -- see
    // SerialMonitorConfigEditor::setConfig()'s own comment for why this
    // matters beyond avoiding wasted work (a reflexive PropertiesPanel
    // refresh landing here mid-combo-teardown).
...
QStringList RobotLogConfigEditor::currentDeviceIds() const {
    QStringList ids;
    for (QComboBox* combo : m_deviceCombos) {
...
void RobotLogConfigEditor::rebuildRows(const QStringList& deviceIds) {
...
void RobotLogConfigEditor::addRowWidget(const QString& deviceId) {
    ...
    combo->setToolTip(tr("Device this tab's log reads from."));
...
void RobotLogConfigEditor::onStructureChanged() {
    if (m_updating) {
        return;
    }
    emit configChanged();
...
void RobotLogConfigEditor::onPickChanged() {
    if (m_updating) {
        return;
    }
    emit configChanged();
