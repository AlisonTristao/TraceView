...
LogViewer::LogViewer(QWidget* parent) : QWidget(parent) {
    ...
    m_table->setHorizontalHeaderLabels({tr("Timestamp (\xC2\xB5s)"), tr("Severity"),
                                        tr("Source ID"), tr("Boot ID"), tr("Sequence"),
                                        tr("Message")});
...
void LogViewer::openFile(const QString& filePath) {
    ...
    for (int row = 0; row < entries.size(); ++row) {
        ...
        m_table->setItem(row, kTimestampColumn,
                         new QTableWidgetItem(QString::number(entry.timestampUs)));
        m_table->setItem(
            row, kSeverityColumn,
        ...
        m_table->setItem(row, kSequenceColumn,
                         new QTableWidgetItem(QString::number(entry.sequence)));
